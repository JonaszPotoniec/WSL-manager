...
let distroList = {
  ...
  template: `
    <div class="box">
      <h1>{{name}}</h1>
      <div v-on:click="setDefault({name})" class="button" style="background-color: rgb(79, 121, 84)">
        set default
      </div>
      <div v-on:click="deleteDistr({name}, {id})" class="button" style="background-color: rgb(121, 79, 79)">
        delete
      </div>
    </div>
  `,
  methods: {
    setDefault: function (distrName) {
      console.log(distrName.name.replace(/(\(Default\))/g, ""));
